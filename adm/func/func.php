...
function validarSessao($redirecionar)
{
    ...
    try {
        ...
        if (isset($_SESSION['idsis']) && !empty($_SESSION['idsis']) && is_numeric($_SESSION['idsis'])) {
            $idAdmin = $_SESSION['idsis'];
            $sqlLista = $conn->prepare("SELECT idsis, nome, foto, cpf, nascimento, email, celular, color "
                . "FROM sis "
                . "WHERE idsis = ? AND ativo = ?");
            $sqlLista->bindValue(1, $idAdmin, PDO::PARAM_INT);
...
function validarSenhaArrayAssociativo($campos, $tabela, $campoBdString, $campoBdString2, $campoParametro, $campoParametro2, $campoBdAtivo, $valorAtivo)
{
    $conn = conectar();
...
function testeTrayCatch()
{
    try {
        ...
        echo 'Erro: ' . $error_message;  // ou var_dump($error_message);

        // Certifique-se de que o caminho do log está correto
        error_log($error_message, 3, 'log/arquivo_de_log.txt');
...
function verificarCampoQuatroParametrosRetornaObjeto($tabela, $campo, $campoId, $campoId2, $campoId3, $campoId4, $campoParamentro, $campoParametro2, $campoParametro3, $campoParametro4)
{
    $conn = conectar();
    try {
        $conn->beginTransaction();
...
function insertAuditoria($idsis, $responsavel, $acao, $idafetado, $nomeafetado, $tipoacao, $tabela, $cadastro)
{
    ...
    try {
        $conn->beginTransaction();
        $sqInsert = $conn->prepare("INSERT INTO auditoria(idsis, responsavel, acao, idafetado, nomeafetado, tipoacao, tabela, cadastro)VALUES(?,?,?,?,?,?,?,?)");
...
function insertTrintaUmId($tabela, $campos, $value1, $value2, $value3, $value4, $value5, $value6, $value7, $value8, $value9, $value10, $value11, $value12, $value13, $value14, $value15, $value16, $value17, $value18, $value19, $value20, $value21, $value22, $value23, $value24, $value25, $value26, $value27, $value28, $value29, $value30, $value31)
{
    $conn = conectar();
    try {
        $conn->beginTransaction();
        ...
        $sqInsert->bindValue(17, $value17, PDO::PARAM_STR);
        $sqInsert->bindValue(18, $value18, PDO::PARAM_STR);
        $sqInsert->bindValue(19, $value19, PDO::PARAM_STR);
        $sqInsert->bindValue(20, $value20, PDO::PARAM_STR);
        $sqInsert->bindValue(21, $value21, PDO::PARAM_STR);
        $sqInsert->bindValue(22, $value22, PDO::PARAM_STR);
        $sqInsert->bindValue(23, $value23, PDO::PARAM_STR);
        $sqInsert->bindValue(24, $value24, PDO::PARAM_STR);
        $sqInsert->bindValue(25, $value25, PDO::PARAM_STR);
        $sqInsert->bindValue(26, $value26, PDO::PARAM_STR);
        $sqInsert->bindValue(27, $value27, PDO::PARAM_STR);
        $sqInsert->bindValue(28, $value28, PDO::PARAM_STR);
        $sqInsert->bindValue(29, $value29, PDO::PARAM_STR);
        $sqInsert->bindValue(30, $value30, PDO::PARAM_STR);
        $sqInsert->bindValue(31, $value31, PDO::PARAM_STR);
        $sqInsert->execute();
        $idInsertRetorno = $conn->lastInsertId();
        $conn->commit();
        if ($sqInsert->rowCount() > 0) {
...
function altUm($tabela, $campoBd, $valorReceberInput, $campoBdId, $inputInivisivelVeioDoInput)
{
    {
        $conn = conectar();
        try {
            $conn->beginTransaction();
            $sqAlterar = $conn->prepare("UPDATE $tabela SET $campoBd = ? WHERE $campoBdId = ? ;");
            $sqAlterar->bindValue(1, $valorReceberInput, PDO::PARAM_STR);
            $sqAlterar->bindValue(2, $inputInivisivelVeioDoInput, PDO::PARAM_INT);
            $sqAlterar->execute();
            $conn->commit();
            if ($sqAlterar->rowCount() > 0) {
                return $sqAlterar;
            } else {
                return null;
            };
...
{
    {
        ...
        try {
            $conn->beginTransaction();
            $sqAlterar = $conn->prepare("UPDATE $tabela SET $campoBd = ?, $campoBd2 =? WHERE $campoBdId = ?");
            $sqAlterar->bindValue(1, $valorReceberInput, PDO::PARAM_STR);
            $sqAlterar->bindValue(2, $valorReceberInput2, PDO::PARAM_STR);
            $sqAlterar->bindValue(3, $inputInivisivelVeioDoInput, PDO::PARAM_INT);
            $sqAlterar->execute();
            $conn->commit();
            if ($sqAlterar->rowCount() > 0) {
                return $sqAlterar;
...
{
    {
        ...
        try {
            $conn->beginTransaction();
            $sqAlterar = $conn->prepare("UPDATE $tabela SET $campoBd = ?, $campoBd2 =?, $campoBd3 =? WHERE $campoBdId = ?");
            $sqAlterar->bindValue(1, $valorReceberInput1, PDO::PARAM_STR);
            $sqAlterar->bindValue(2, $valorReceberInput2, PDO::PARAM_STR);
            $sqAlterar->bindValue(3, $valorReceberInput3, PDO::PARAM_STR);
            $sqAlterar->bindValue(4, $inputInivisivelVeioDoInput, PDO::PARAM_INT);
            $sqAlterar->execute();
            $conn->commit();
            if ($sqAlterar->rowCount() > 0) {
                return $sqAlterar;
...
function upUm($tabela, $campo1, $campoId, $valor1, $valorId)
{
    ...
    try {
        ...
        $sqUpdate->execute([$valor1, $valorId]);

        $linhasAfetadas = $sqUpdate->rowCount();
