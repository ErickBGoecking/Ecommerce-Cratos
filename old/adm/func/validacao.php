...
function verificarUsuarioSenha($email, $senha)
{
    ...
    try {
        ...
        $sqlLoginEmail = $conn->prepare("SELECT idsis, email, senha, ativo FROM sis WHERE email = ? ");
        ...
        if ($sqlLoginEmail->rowCount() == 1) {
            ...
            $idsis = $rsLoginEmail["idsis"];
            $ativo = $rsLoginEmail["ativo"];
            if ($ativo == 'A') {
                if (password_verify($senha, $senhaCadastro)) {
                    $_SESSION['idsis'] = $idsis;
                    falhaLoginFalse($idsis);
                    insertContadorAcesso($idsis);
                    echo json_encode('loginTrue');
                } else {
                    $tentativas = verificaFalhaLogin($idsis);
...
function verificaFalhaLogin($idusuario)
{
    ...
    try {
        $conn->beginTransaction();
        $sqlVerificaLoginFalha = $conn->prepare("SELECT falha, tempo FROM sis WHERE idsis = ? ");
...
function insertContadorAcesso($idadministrador)
{
    $conn = conectar();
    try {
        $ipusuario = pegaip();
        $conn->beginTransaction();
        $sqlInsertContador = $conn->prepare("INSERT INTO sisacesso (idsis, cadastro, ip) VALUES(?, ?, ?)");
        $sqlInsertContador->bindValue(1, "$idadministrador", PDO::PARAM_INT);
        $sqlInsertContador->bindValue(2, DATATIMEATUAL, PDO::PARAM_STR);
        $sqlInsertContador->bindValue(3, "$ipusuario", PDO::PARAM_STR);
...
function falhaLogin($idusuario)
{
    ...
    try {
        ...
        $sqlLoginFalha = $conn->prepare("UPDATE sis SET falha = falha + 1, tempo = DATE_ADD(CURRENT_TIMESTAMP, INTERVAL $tempoFalha MINUTE) WHERE idsis = ? ");
...
function falhaLoginFalse($idusuario)
{
    ...
    try {
        $conn->beginTransaction();
        $sqlLoginFalha = $conn->prepare("UPDATE sis SET falha = 0,tempo = CURRENT_TIMESTAMP WHERE idsis = ? ");
        $sqlLoginFalha->bindValue(1, $idusuario, PDO::PARAM_INT);
        $sqlLoginFalha->execute();
        $conn->commit();
    } catch
